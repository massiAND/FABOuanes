...
def test_mobile_connect_uses_lan_ip_when_opened_from_localhost(monkeypatch):
    ...
    monkeypatch.setattr(mobile_connect, "_get_local_ip", lambda: "192.168.1.44")

    assert mobile_connect.resolve_mobile_connect_url(_request()) == "http://192.168.1.44:5000"
...
def test_mobile_connect_context_builds_qr_for_lan_url(monkeypatch):
    ...
    assert context["mobile_connect_url"] == "http://192.168.1.44:5000"
    assert str(context["mobile_connect_qr_uri"]).startswith("data:image/png;base64,")
