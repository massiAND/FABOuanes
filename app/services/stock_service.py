...
def create_purchase_record(
    ...
    with db_transaction():
        ...
        if purchase_date and purchase_date > date.today().isoformat():
            raise ValueError("La date d'achat ne peut pas etre dans le futur.")
...
def create_sale_record(
    ...
    if requested_sale_type == "credit" and not client_id:
        raise ValueError("Une vente a credit necessite un client.")
    ...
    if sale_date and sale_date > date.today().isoformat():
        raise ValueError("La date de vente ne peut pas etre dans le futur.")
